//
//  ErrorDataViewController.m
//  AddressFlu
//
//  Created by 吴小星 on 2017/6/6.
//  Copyright © 2017年 crash. All rights reserved.
//

#import "ErrorDataViewController.h"

@interface ErrorDataViewController ()
...
@implementation ErrorDataViewController
...
-(void)rightBtn:(UIButton *)button{
    ...
    //读取本地csv 文件路径
    NSString *path = [[NSBundle mainBundle] pathForResource:@"坐标重复的 数据 2" ofType:@"csv"];
    ...
    if (csvContent.count > 0) {
        ...
        self.totalLb.text = [NSString stringWithFormat:@"数据总行:%ld",csvContent.count-2];
        ...
        for (int c = 2; c < [csvContent count]; c++) {
            
            //获取每一行csv数据流
            NSMutableArray *array = [csvContent objectAtIndex:c];
            
            if (array.count > 2 && ![array[1] isEqualToString:@""]) {
                ...
                [[GDCodeHandler alloc] gecodeAddress:array[1] andCity:array[5] andCSVData:array andSuccess:^(GeCodeListModel * _Nonnull codeModel) {
                    ...
                    [[GDCodeHandler alloc] gecodeAddress:array[2] andCity:array[5]  andCSVData:csvData andSuccess:^(GeCodeListModel * _Nonnull codeModel) {
...
-(void)opposite:(NSString *_Nonnull) location andCSVdata:(NSMutableArray *_Nonnull)csvData andFilePath:(NSString *_Nonnull) filePath{
    ...
    [[GDGetCodeHandler alloc]getCode:location andCSVData:csvData andSuccess:^(SaveCSVDataModel * _Nonnull saveCSVDataModel) {
        ...
        if (addressModel.township) {
            saveCSVDataModel.csvData[8] = addressModel.township;
        }else{
...
//        }
        ...
        //查询药店名称存在不
        [[GDCodeHandler alloc] gecodeAddress:saveCSVDataModel.csvData[1] andCity:saveCSVDataModel.csvData[5] andCSVData:saveCSVDataModel.csvData andSuccess:^(GeCodeListModel * _Nonnull codeModel) {
            
            

            
                   // POI_ID,NAME,LOCATION,COORD,PNAME,CNAME,DNAME,CTIME,VNAME,STREET,HNUMBER,STATUS,COORD_BAIDU,UTIME,GEOHASH,COORD_GAODE\r\n
            NSMutableArray *temp = codeModel.csvData;
            
            GeCodeModel *model = codeModel.codeModel;
            if ([model.level isEqualToString:@"兴趣点"] || [model.level isEqualToString:@"门牌号"]  ||[model.level isEqualToString:@"单元号"]) {
                temp[11] = @"1";
            }else{
                temp[11] = @"1";
            }
            ...
            if (temp && temp.count > 15){
                
                [strong tranfromBD:temp[15] andCSV:temp andFile:filePath];
            }
            ...
            if (csvData && csvData.count > 15){
                
                [strong tranfromBD:csvData[15] andCSV:csvData andFile:filePath];
            }
...
-(void)tranfromBD:(NSString *_Nonnull) lonlat andCSV:(NSMutableArray *_Nonnull)csv andFile:(NSString *_Nonnull)filePath{
    ...
    [[BDCodeTransHandler alloc] bdLocationTran:lonlat andFrom:3 andCSVData:csv andSuccess:^(BDLocationTranReturnModel * _Nonnull bdLocationModel) {
        ...
        NSString *lonlatd = bdcsvData[15];
        
        [strong tranformWGS84:bdcsvData[15] andCSV:bdcsvData andFile:filePath];
        ...
        //转换wgs坐标系
        [self tranformWGS84:csvData[15] andCSV:csvData andFile:filePath];
...
-(void)exportCSVHeader:(NSString *)filePath{
    ...
    //表头数据
    NSString *header = @"POI_ID,NAME,LOCATION,COORD,PNAME,CNAME,DNAME,CTIME,VNAME,STREET,HNUMBER,STATUS,COORD_BAIDU,UTIME,GEOHASH,COORD_GAODE\r\n";
