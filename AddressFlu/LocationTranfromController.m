...
#import "LocationTranfromController.h"

@interface LocationTranfromController ()
...
@implementation LocationTranfromController
...
-(void)rightBtn:(UIButton *)button{
    ...
    //读取本地csv 文件路径
    NSString *path = [[NSBundle mainBundle] pathForResource:@"address" ofType:@"csv"];
...
-(void)tranfromBD:(NSString *_Nonnull) lonlat andCSV:(NSMutableArray *_Nonnull)csv andFile:(NSString *_Nonnull)filePath{
    ...
    [[BDCodeTransHandler alloc] bdLocationTran:lonlat andFrom:1 andCSVData:csv andSuccess:^(BDLocationTranReturnModel * _Nonnull bdLocationModel) {
